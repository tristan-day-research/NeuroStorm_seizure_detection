...
# # Dummy codebook initialization
# codebook_size = 1024
# emb_dim = 64
# # Creating a dummy codebook tensor with random values

# dummy_codebook = torch.randn(codebook_size, emb_dim)
# dummy_codebook.to(device)


# quantize=True
# use_saved_codebook=False

# test_model = VQVAE_v1_w_codebook(dummy_codebook, codebook_size=codebook_size, emb_dim=emb_dim,  patch_size=200, quantize=quantize, use_saved_codebook=use_saved_codebook)  # Start with quantize=False


# for name, module in test_model.named_modules():
#     if isinstance(module, nn.Conv2d) or isinstance(module, nn.ConvTranspose2d):
#         module.register_forward_hook(print_size_hook)

# test_model.to(device)

# # Example forward pass with dummy data shaped [16, 19, 1000, 200]
# x_dummy = torch.randn(2, 19, 1000, 200)  # Example input
# x_dummy = x_dummy.to(device)

# print("Input shape", x_dummy.shape)

# quantized, decoded = test_model(x_dummy)  # Forward pass
